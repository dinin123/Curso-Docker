...
- [Ejercicio 4: Contenedor Jaula SSH con acceso por certificado](#ejercicio-4-contenedor-jaula-ssh-con-acceso-por-certificado)
...
Partiremos del siguiente directorio:
...
.:
total 12
drwxr-xr-x  3 root root   67 Jun  3 07:23 .
drwxr-xr-x 21 root root 4096 Apr 14 12:16 ..
-rw-r--r--  1 root root  406 Jun  3 07:23 docker-compose.yml
-rwxr-xr-x  1 root root  563 Jun  3 07:21 init-ssh.sh
drwxr-xr-x  2 root root   71 Jan 10 08:54 ssh-keys

./ssh-keys:
total 12
drwxr-xr-x 2 root root   71 Jan 10 08:54 .
drwxr-xr-x 3 root root   67 Jun  3 07:23 ..
-rw------- 1 root root 2031 Jul  2  2024 authorized_keys
-rw------- 1 root root  806 Aug 19  2024 known_hosts
-rw-r--r-- 1 root root  142 Aug 19  2024 known_hosts.old
```

Tenemos nuestro fichero de compose "docker-compose.yml":

```bash
services:
  ssh-service:
    image: alpine:latest
    container_name: jaulassh
    volumes:
      - ./ssh-keys:/root/.ssh:ro  # Montar las claves públicas
      - ./init-ssh.sh:/init-ssh.sh:ro  # Montar el script de inicialización
    ports:
      - "2222:22"  # Exponer puerto 2222 al exterior
    entrypoint: ["/bin/sh", "/init-ssh.sh"]
    restart: always
```

El fichero init-ssh.sh para configurar y lanzar ssh.
...
adduser -D -u 1001 -G mygroup -s /bin/sh myuser

# Configurar SSH
mkdir -p /home/myuser/.ssh
chown 1001:1001 /home/myuser/.ssh
cp /root/.ssh/authorized_keys /home/myuser/.ssh/
chown -R myuser:mygroup /home/myuser/.ssh
...
# Ejecutar el servicio SSH en primer plano
exec /usr/sbin/sshd -D
```

Y finalmente el directorio con las clave:
...
Lanzamos el compose y verificamos:
...
ebee251157f2   alpine:latest   "/bin/sh /init-ssh.sh"   32 seconds ago   Up 31 seconds   0.0.0.0:2222->22/tcp   jaulassh
